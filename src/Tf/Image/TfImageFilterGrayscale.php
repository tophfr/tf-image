 ...
 */
class TfImageFilterGrayscale extends TfImageFilter
{

    var $ncolors;

    function __construct($ncolors = 256)
    {
    ...
    function apply($img)
    {
        ...
        imagetruecolortopalette($new, true, $this->ncolors);

        $total = imagecolorstotal($new);
        for ($i = 0; $i < $total; $i++) {
            $c = imagecolorsforindex($new, $i);
            $g = ($c['red'] + $c['green'] + $c['blue']) / 3;
            imagecolorset($new, $i, $g, $g, $g);
        }
        return $new;
    }
    ...
    {
        return function_exists('imagetruecolortopalette');
    }
}
